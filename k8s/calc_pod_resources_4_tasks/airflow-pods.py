...
prom_request_url = "http://prometheus.monitoring.analytics.example.com/api/v1/query"
...
def get_tasks_pod_query():
    ...
    query_str = 'sum_over_time(kube_pod_info{namespace="airflow",pod!~"airflow-.*"}[14d:])'
    query = { "query" : query_str }
    return query

def get_cpu_query(pod):
    query_str = 'max(max_over_time(rate(container_cpu_usage_seconds_total{{namespace="airflow",pod=~"{}-.*"}}[5m]) [20d:5m]))'.format(pod)
    query = { "query" : query_str }
    return query

def get_mem_query(pod):
    query_str = 'max(max_over_time(container_memory_max_usage_bytes{{namespace="airflow",pod=~"{}-.*"}} [20d:5m])) /(1024* 1024)'.format(pod)
    query = { "query" : query_str }
    return query
...
def make_pod_tasks_list():
    ...
    for i in raw_data:
        ...
        if splited_pods[0] == "barcodemodel":
            pod_name = "barcodemodel"
        else:
            pod_name= '-'.join(splited_pods[:-2])
        if pod_name in ["update","delete-trash-pods"]:
            pass
        else:
            if pod_name in tasks_pods :
                pass
            else:
                tasks_pods.append(pod_name)
...
if __name__ == '__main__':
    ...
    if config_type == "pod_type" :
        ...
        for task in tasks:
            pod_resources[task] = get_pod_resources(task)
    print(pod_resources)
